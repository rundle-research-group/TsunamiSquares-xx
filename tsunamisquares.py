...
def make_animation(sim_data, FPS, DPI, T_MIN, T_MAX, T_STEP, N_STEP):
    ...
    z_min,z_max = sim_data['z'].min(), sim_data['z'].max()#-sim_data['z'].std(), sim_data['z'].std()#-.1, .1#
...
#    MODE = "eq_field_eval"
    ...
    if MODE == "generate": #read bethymetry file
        ...
        system("python "+VQ_DIR+"vq/PyVQ/pyvq/pyvq.py --field_eval  --event_file {} --model_file {} --event_id {} --lld_file {} ".format(EVENTS, MODEL, EVID, SAVE_NAME))
...
#        make_map_animation(sim_data, FPS, DPI, T_MIN, T_MAX, T_STEP, N_STEP, save_file)
    ...
    if MODE == "eq_field_eval":
        ...
        EVID      = 39951
        # ---- compute field and write it ------
        system("python "+VQ_DIR+"vq/PyVQ/pyvq/pyvq.py --field_eval  --netCDF --event_file {} --model_file {} --event_id {} --lld_file {} ".format(EVENTS, MODEL, EVID, LLD_NAME))
